...
export default function AboutPage() {
  return (
    <>
      <section className="relative overflow-hidden bg-navy py-16 md:py-24"><div className="blueprint-grid absolute inset-0" /><div className="page-shell relative max-w-4xl"><p className="eyebrow text-slate-200">About TriCore Surgical</p><h1 className="text-4xl font-extrabold leading-tight text-white md:text-5xl">A focused source for clinical equipment and healthcare furniture.</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-slate-100">TriCore Surgical serves wholesale and retail buyers seeking dependable general surgical equipment and hospital furniture. Our role is to make product discovery and quote requests direct, informed, and responsive.</p></div></section>
      ...
      <section className="relative overflow-hidden bg-steel py-16 md:py-24"><div className="blueprint-grid absolute inset-0" /><div className="page-shell relative grid gap-10 lg:grid-cols-[1fr_auto] lg:items-center"><div className="max-w-2xl"><p className="eyebrow text-slate-200">Brand partnerships</p><h2 className="text-3xl font-bold text-white">A catalogue backed by trusted names.</h2><p className="mt-4 leading-7 text-slate-100">Our range includes products associated with Atom Medical, LifeCare Medical, Yuwell, UCheck, and LifeChek. Explore our partners or send a detailed product requirement to discuss availability.</p><div className="mt-6 flex flex-wrap gap-4"><ButtonLink href="/brands" variant="light">Explore Brand Partners <ArrowRight size={16} className="ml-2" /></ButtonLink><Link href="/quote" className="inline-flex items-center font-heading text-sm font-bold text-white hover:text-slate-200">Discuss a requirement</Link></div></div><Handshake className="hidden lg:block text-white/20" size={88} /></div></section>
    </>
  );
}
